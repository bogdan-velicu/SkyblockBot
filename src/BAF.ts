...
import {
  claimSoldItem,
  registerIngameMessageHandler,
} from "./ingameMessageHandler";
import { MyBot, TextMessageData } from "../types/autobuy";
import {
  getConfigProperty,
  initConfigHelper,
  updatePersistentConfigProperty,
} from "./configHelper";
  ...
  bot._client.on("packet", async function (packet, packetMeta) {
    if (packetMeta.name.includes("disconnect")) {
      ...
      printMcChatToConsole(
        "§f[§4BAF§f]: §fYou were disconnected from the server...",
      );
...
function connectWebsocket(url: string = getConfigProperty("WEBSOCKET_URL")) {
  ...
  _websocket = new WebSocket(
    `${url}?player=${bot.username}&version=${version}&SId=${getSessionId(ingameName)}`,
  );
...
async function onWebsocketMessage(msg) {
  ...
  switch (message.type) {
    ...
    case "chatMessage":
      if (
        data.length > 1 &&
        data[1].text.includes("matched your Whitelist entry:") &&
        !isCoflChatMessage(data[1].text)
      ) {
    ...
    case "trade":
      ...
      break;
    case "tradeResponse":
      let tradeDisplay = (bot.currentWindow.slots[39].nbt.value as any).display
        .value.Name.value;
    ...
    setTimeout(async () => {
      ...
      wss.send(
        JSON.stringify({
          ...
          data: JSON.stringify(
            bot.scoreboard.sidebar.items.map((item) =>
              item.displayName.getText(null).replace(item.name, ""),
            ),
          ),
        }),
      );
